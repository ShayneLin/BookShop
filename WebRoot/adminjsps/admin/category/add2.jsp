...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/admin/adminCategoryAction_add'/>" method="post" onsubmit="return checkForm()">
    	...
    	一级分类：<select name="pid" id="pid">
    		...
    		<c:forEach items="${parents}" var="item">
    			<option value="${item.cid }"  <c:if test="${pid eq item.cid }"> selected="selected"</c:if>>${item.cname }</option>
    		</c:forEach>
    	</select><br/>
