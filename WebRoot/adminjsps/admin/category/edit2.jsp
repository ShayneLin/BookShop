...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/admin/adminCategoryAction_editChild'/>" method="post" onsubmit="return checkForm()">
    	...
    	一级分类：<select name="pid" id="pid">
	    	...
	    	<c:forEach items="${parents }" var="item">
	    		<option value="${item.cid}" <c:if test="${item.cid eq pid }">selected="selected"</c:if>>${item.cname }</option>
	    	</c:forEach>
    	</select><br/>
