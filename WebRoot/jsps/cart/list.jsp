...
<script type="text/javascript">
						...
						function() {
							...
							if (quantity == 1) {
								if (confirm("您是否真要删除该条目？")) {
									location = "/myshop/cartItemAction_batchDelete?cartItemIds="
											+ id;
								}
							} else {
	...
	function sendUpdateQuantity(id, quantity) {
		$.ajax({
			async : false,
			cache : false,
			url : "/myshop/cartItemAction_updateQuantity",
			data : {
				cartItemId : id,
				quantity : quantity
			},
...
<body>
	...
	<c:choose>
		...
		<c:otherwise>
				...
				<c:forEach items="${cartItemList }" var="cartItem">
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/bookAction_load?bid=${cartItem.book.bid }'/>"><img border="0"
								width="54" align="top"
								src="<c:url value='/${cartItem.book.image_b }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/bookAction_load?bid=${cartItem.book.bid }'/>"><span>${cartItem.book.bname }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice">${cartItem.book.currPrice }</span></span></td>
						<td><a class="jian" id="${cartItem.cartItemId }Jian"></a>
						<input	class="quantity" readonly="readonly" id="${cartItem.cartItemId }Quantity" type="text" value="${cartItem.quantity }" /><a class="jia"
						...
						<td><a
							href="<c:url value='/cartItemAction_batchDelete?cartItemIds=${cartItem.cartItemId }'/>">删除</a>
						</td>
					</tr>
				</c:forEach>
			...
			<form id="jieSuanForm" action="<c:url value='/cartItemAction_loadCartItems'/>"
				method="post">
